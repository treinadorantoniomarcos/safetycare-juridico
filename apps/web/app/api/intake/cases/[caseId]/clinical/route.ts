import { ClinicalAnalysisRepository, CaseRepository } from "@safetycare/database";
...
export async function GET(request: Request, context: RouteContext) {
  ...
  try {
    ...
    const clinical = await clinicalAnalyses.findByCaseId(caseId);

    if (!clinical) {
      return NextResponse.json(
        {
          correlationId,
          error: "clinical_not_found"
        },
        { status: 404 }
      );
    }
